...
import { IonList, MenuController, ModalController, AlertController, LoadingController, PopoverController } from '@ionic/angular';
...
import { CompartirdocComponent } from '../../components/compartirdoc/compartirdoc.component';
...
export class ListaPage implements OnInit {
  ...
  list: any;
  pdfObj = null;
  constructor(private menu: MenuController, private router: Router, private modalCtrl: ModalController, private dataServices: DataService, private alertCtrl:AlertController, private loadingCtrl:LoadingController,private popoverCtrl:PopoverController) { }
  ...
  cuerpolista(id) {
    console.log(id);
    this.router.navigate(['listverificacion'],id);
 ...
 async eliminarLista(id_lf: string) {

    const alert = await this.alertCtrl.create({
      header: 'Confirmación',
      message: '¿Estás seguro que desea eliminar el documento?',
      buttons: [
        {
          text: 'Si',
          handler: async () => {
...
async  compartirLista(evento) {
  
    this.lista.closeSlidingItems();

    const popover = await this.popoverCtrl.create({
      component: CompartirdocComponent,
      event:evento,
      mode:'ios'
    });
